...
<details>
<summary> 2/28 <5일차> </summary>

- 일단 double은 15~16자리까지만 정확하다. 그래서 n이 크면 오류가 난다. 
그리고 소수점 n번째 자리를 구하기 위해서 0.1을 나눈 값을 뺐는데 공식이 따로 있었다. "a / b = 몫 + 나머지 / b"이다. 너무 무식하게 문제를 푼거같다. 디테일을 챙기지 못했다.

</details>

<details>
<summary> 3/1 <6일차> </summary>

- 큐가 순환하면서 우선순위 체크하는 로직은 이해해서 deque로 풀어보려고 했는데 너무 코드가 길어져서 뭔가 이상함을 느꼈다. 초초초단순하게 다시 생각해서 우선순위가 다르면 큰 숫자대로 순위를 매기면 되고, 같으면 큰 숫자 뒤에 같은 순위가 몇개있는지 체크하면 되는거 아닌가? 했지만 그런 문제는 아니였다. 챗지피티와 대화한 결과, 이 문제는 우선순위큐(priority queue)와 pair 사용법을 활용하여 풀었어야했다. pair에 입력받은 우선순위와 인덱스를 한쌍으로 저장하고, pair 큐를 순환시키면서 pair의 front와 우선순위큐의 top이 일치하는지 비교하는 로직으로 가야한다. 어렵다. 

</details>

<details>
<summary> 3/2 <7일차> </summary>

- 일단 만들어야하는 정답 수열을 vector로 두고 1 ~ n의 수열을 stack으로 둔건 맞았다. 하지만 while문과 조건문 설정을 잘못했다. 만들고 있는 스택의 top이 정답 배열의 현재 값보다 크면 만들 수 없기때문에 NO를 출력해야한다. 같다면 pop하고 작다면 인덱스 값을 키우며 계속 push한다. 아래의 게시판 코드를 참고하여 풀었다.
https://www.acmicpc.net/board/view/161842
